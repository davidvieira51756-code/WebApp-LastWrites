...
class AzureVaultKeyService:
    def __init__(self, key_vault_url: Optional[str] = None) -> None:
        ...
        self._key_size = int(os.getenv("KEY_VAULT_RSA_KEY_SIZE", "2048"))
        self._hardware_protected = _env_to_bool(os.getenv("KEY_VAULT_RSA_HARDWARE_PROTECTED", "false"))

    @staticmethod
    def _public_jwk_from_key(key: Any) -> Dict[str, str]:
        key_jwk = key.key
        ...
        return public_jwk_from_rsa_public_key(public_numbers.public_key())
    ...
    def ensure_vault_key(self, vault_id: str) -> Dict[str, Any]:
        ...
        public_jwk = self._public_jwk_from_key(key)
        return {
            "key_name": key_name,
            "key_kid": str(key.id),
...
class LocalVaultKeyService:
    ...
    def ensure_vault_key(self, vault_id: str) -> Dict[str, Any]:
        ...
        key_name = _build_key_name(vault_id)
        private_key_path = self._private_key_path(key_name)
